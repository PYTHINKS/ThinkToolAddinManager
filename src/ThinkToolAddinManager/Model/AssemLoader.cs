...
namespace ThinkToolAddinManager.Model;
...
public class AssemLoader
{
    ...
    public void CopyGeneratedFilesBack()
    {
        if (string.IsNullOrEmpty(tempFolder) || !Directory.Exists(tempFolder) ||
            string.IsNullOrEmpty(originalFolder) || !Directory.Exists(originalFolder))
        {
            return;
        }
    ...
    public Assembly LoadAddinsToTempFolder(string originalFilePath, bool parsingOnly)
    {
		...
		AddReferencedFolder(Path.GetDirectoryName(originalFilePath));
    ...
    private string ResolveDuplicateMethod(string originalFilePath)
    {
        ...
        var ass = GetAssemblyDef(originalFilePath);
        try
        {
            foreach (ModuleDefinition def in ass.Assembly.Modules)
            {
            ...
            ass.Assembly.Write(fileAssemblyTemp, new WriterParameters { WriteSymbols = ass.HasSymbols });
            return fileAssemblyTemp;
        }
        finally
        {
            ass.Assembly.Dispose();
        }
    }

    private sealed class AssemblyReadResult
    {
        public AssemblyReadResult(AssemblyDefinition assembly, bool hasSymbols)
        {
            Assembly = assembly;
    ...
    private static AssemblyReadResult GetAssemblyDef(string assemblyPath)
    {
        ...
        var hasSymbols = File.Exists(pdbName);
        if (hasSymbols)
        {
        ...
        return new AssemblyReadResult(assemblyDef, hasSymbols);
    }
    ...
    private Assembly CopyAndLoadAddin(string srcFilePath, bool onlyCopyRelated)
    {
        ...
        if (!FileUtils.FileExistsInFolder(srcFilePath, tempFolder))
        {
            ...
            AddReferencedFolder(directoryName);
    ...
    private Assembly LoadAddin(string filePath)
    {
        ...
        {
            Monitor.Enter(this);
            // Loading is serialized to avoid dependency load conflicts.
    ...
    private void AddReferencedFolder(string folder)
    {
        if (!string.IsNullOrEmpty(folder) && !refedFolders.Contains(folder))
        {
            refedFolders.Add(folder);
        }
    }
